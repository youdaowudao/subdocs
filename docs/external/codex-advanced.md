...
本页主要讲系统终端里的外部 `codex ...` 命令、高级配置和多会话工作流。已经进入 Codex 交互界面后，输入框里的 `/goal`、`/fork`、`/review`、`/model` 等内部命令，请看 [Codex CLI 内部命令](/external/codex-cli-internal-commands)。

以下内容基于当前本机可验证的 `codex-cli 0.142.4` 写作。不同版本的 Codex CLI 可能会增减命令、配置项和本地目录结构；遇到差异时，优先以本机 `codex --help`、Codex 输入框里的 `/` 菜单、`codex doctor` 和 UseGoodAI 后台生成配置为准。
...
这些是外部 CLI 命令，不等于进入 Codex 之后输入的内部 slash command。例如 `/goal` 是交互界面里的内部命令，不是 `codex goal`；`/fork` 是当前会话里的分叉入口，`codex fork --last` 是从系统终端启动分叉会话。
...
如果使用 UseGoodAI，优先复制后台生成的 Codex 配置，不要在高级配置里重新推导 provider 字段。UseGoodAI API 地址通常是 `https://api.usegoodai.com`；Codex 接入优先走 Responses。需要手动排障时，检查后台生成配置和基础接入页，不要把 Base URL 改成 Responses 或 Chat Completions 的最终请求路径。
...
如果想为不同项目、不同客户或不同账号隔离配置，可以显式指定不同的 `CODEX_HOME`：
...
| 临时约束 | 启动 prompt | 给当前任务的目标、范围和验收标准 |
...
建议把稳定规则写进仓库里的 `AGENTS.md`，把单次任务目标写在 prompt 里。不要把所有背景一次性塞进 prompt；上下文越混杂，Codex 越难判断优先级。
...
Codex 会保存历史会话，便于继续、分叉或清理。

| 命令 | 适合场景 |
| --- | --- |
| `codex resume` | 从列表选择历史会话继续 |
| `codex resume --last` | 继续最近一次会话 |
| `codex fork` | 从某个历史会话分叉新方向 |
| `codex fork --last` | 从最近一次会话分叉 |
| `codex archive <SESSION>` | 归档指定历史会话 |
| `codex delete <SESSION>` | 删除指定历史会话 |
| `codex unarchive <SESSION>` | 恢复指定归档会话 |

典型工作流：

```bash
codex resume --last
codex fork --last "不要继续实现，改为只做代码审查"
```

`resume` 适合延续原目标，`fork` 适合保留原上下文但尝试另一个方向。例如一个会话已经分析了支付模块，可以 fork 出“修 bug”和“写测试计划”两条线。

`resume` 和 `fork` 不带参数时会打开选择列表；`archive`、`delete`、`unarchive` 需要明确传入 session id 或 session name。删除会话前先确认目标 session，避免误删。
...
联网搜索会增加外部信息摄入，也可能增加消耗。涉及账号、账单、私有仓库、客户资料时，优先使用受控 MCP 或本地文档，不要把敏感信息直接放进联网搜索 prompt。
...
如果需要让 Codex 读取或修改主项目之外的目录，优先用 `--add-dir` 精确授权，不要直接扩大到整个用户目录。
...
```bash
codex fork --last "基于刚才的分析，只修复登录 bug"
codex fork --last "基于刚才的分析，只补测试计划，不改代码"
```

并发会增加模型调用、工具调用和搜索消耗，也会增加冲突概率。多个 Codex 同时修改同一个仓库时，建议先按目录或文件划分边界，并在每条线结束后统一检查 diff。

## 目标管理

目标管理的核心是让 Codex 始终知道“完成标准”，不只是知道“要做什么”。注意：`/goal` 是进入 Codex 交互界面后的内部命令，不是系统终端里的 `codex goal` 子命令。内部命令的详细用法看 [Codex CLI 内部命令](/external/codex-cli-internal-commands)。
...
目标管理不需要复杂系统。能稳定复用的规则放进 `AGENTS.md`，临时目标可以用 `/goal` 或写进 prompt，历史上下文用 `resume`，分支探索用 `fork`。
