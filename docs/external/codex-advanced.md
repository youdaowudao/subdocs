...
本页主要讲系统终端里的外部 `codex ...` 命令、高级配置、多会话工作流和 Codex App 手机连接。已经进入 Codex 交互界面后，输入框里的 `/goal`、`/fork`、`/review`、`/model` 等内部命令，请看 [Codex CLI 内部命令](/external/codex-cli-internal-commands)。

以下内容基于当前本机可验证的 `codex-cli 0.142.5` 写作。外部命令、配置项和本地目录结构会随版本变化；与本机显示不一致时，以本机 `codex --help`、`codex doctor` 和 UseGoodAI 后台生成配置为准。
...
这些是外部 CLI 命令，不等于进入 Codex 之后输入的内部 `/...` 命令。例如 `/goal` 是交互界面里的内部命令，不是 `codex goal`；`/fork` 是当前会话里的分叉入口，`codex fork --last` 是从系统终端启动分叉会话。
...
Codex App 保持 ChatGPT 登录后，手机端可以扫码连接当前电脑。电脑继续保留项目文件、终端环境、权限和长任务；手机负责查看进度、补充要求、让任务继续跑。

完整配置看 [保持 ChatGPT 登录同时连接中转站](/chatgpt-login-usegoodai)。进阶用法只记这几步：

1. 电脑端打开 Codex App 设置里的 **连接**。
2. 在 **控制此电脑** 页面点击 **添加**。
3. 手机端登录 ChatGPT 后扫描二维码。
4. 长期外出使用时，打开 **使此电脑保持唤醒**。
5. 不再使用某台设备时，点击 **撤销访问权限**。
