...
本页适合已经完成 Codex 基础安装和 UseGoodAI 接入的用户。首次配置 API Key、安装 CLI、Windows / WSL / macOS / Linux 路径排查，请先看 [Codex 接入](/clients/codex)。
...
完整配置看 [保留 ChatGPT 登录](/clients/chatgpt-login-usegoodai)。进阶用法只记这几步：
