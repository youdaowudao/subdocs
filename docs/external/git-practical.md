...
Git 最常见的用途是管理代码版本，但它本质上不是“只给程序员写代码用的工具”。它解决的是一个更通用的问题：**一份文件改来改去，怎么知道改了什么、什么时候改的、能不能回到以前、能不能和别人同步**。

对代码编辑来说，Git 几乎是基础设施。AI 可以帮你改代码、修 bug、重构文件、生成测试，但这些修改必须有边界：改了哪些文件、哪里是 AI 改的、能不能审查、能不能撤回。Git 就是给 AI 编程加安全边界的工具。

同时，Git 也可以用在很多非代码场景里：
...
这篇不把重点放在背命令上。更实用的做法是：你把目标说清楚，让 AI 帮你检查当前状态、执行 Git 操作、解释结果。下面的提示词可以直接复制给 Codex、ChatGPT、豆包、DeepSeek 或其它能操作你本地文件的 AI 工具。

::: warning 先说清楚边界
如果你不熟悉 Git，在提示词里要明确禁止 AI 直接执行 `reset --hard`、`git clean -fd`、强制推送等破坏性操作。需要删除、覆盖、回退大量文件时，必须让 AI 先解释影响，再等你确认。
:::
...
```text
请先检查当前目录是不是一个 Git 仓库。
如果不是，只说明当前状态，不要立刻初始化。
如果是，请告诉我当前分支、是否有未保存改动、远端仓库是否已配置。
不要修改任何文件。
```

这一步的意思是先摸清楚环境。Git 仓库一般会有一个隐藏的 `.git` 目录；如果已经是仓库，就能看到当前分支、文件改动和远端地址。先检查再操作，可以避免在错误目录里初始化或提交。
...
`git add` 不是最终保存，它只是把文件放进“准备提交”的清单。代码项目里建议按文件或按主题添加，不要不检查就直接提交所有文件。
...
真正使用时，可以让 AI 先解释每一步会做什么，再执行。你不需要把所有命令都背下来，但要知道 AI 运行这些命令是在检查状态、查看差异、选择文件、保存版本、同步远端。
...
如果确实需要这些操作，提示词里要加一句：**先解释影响，不要执行，等我确认后再做**。
