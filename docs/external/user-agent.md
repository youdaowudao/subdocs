...
> 本页只说明外接调用时 `User-Agent` 怎么理解、什么时候需要管、以及遇到 `403 block` / `403 Forbidden` 时应该放在排查链路的哪一步。

## 先看结论

`User-Agent` 是客户端发请求时带上的一段请求头，用来说明“这次请求大概来自什么客户端”。多数客户端会自动带上自己的 `User-Agent`，用户不需要手动填写。
...
| 能不能随便复制别人的 UA？ | 不建议。只在确实需要自定义 Header 时，填写稳定、可识别、和当前客户端匹配的标识。 |
...
你只在这些情况需要关注 `User-Agent`：

| 场景 | 要不要处理 | 先做什么 |
| --- | --- | --- |
| 客户端已经能正常回复 | 不需要 | 保持默认配置 |
| 客户端报 `403 block` / `403 Forbidden` | 可能需要 | 先按 403 排查顺序确认 Key、Base URL、模型和接口路径 |
| 客户端支持自定义 Headers | 视情况 | 只有后台记录、代理日志或客服排查需要时再填写 |
| 使用自建代理、脚本、网关转发 | 需要确认 | 检查最终发到 UseGoodAI 的请求头是否被清空或覆盖 |
| 浏览器前端直接请求 | 通常不能手动改 | 先确认浏览器和后端代理实际发出的 Header |

## 速查表

| 接入类型 | 常见工具 | `User-Agent` 处理口径 | 常见优先检查项 |
| --- | --- | --- | --- |
| Codex / Responses | Codex CLI、Codex App、支持 Responses API 的代码 Agent | 通常使用 Codex 自带 UA；不要改成普通聊天客户端 UA | 是否走 Responses，后台记录是否出现 `/v1/responses` |
...
| 浏览器型客户端 | Web 面板、浏览器扩展、自建前端 | 通常使用浏览器真实 UA；前端未必能手动设置 | 浏览器限制、后端代理、CORS 和最终 Header |
...
3. 确认后台请求记录里的接口路径是 `/v1/responses`。
4. 如果前面都正确，仍然 `403 block`，再看最终请求里是否带有 Codex 自带的 `User-Agent`。
...
| Header 转发 | 如果经过网关或代理，确认最终请求没有丢掉 Header |

只有在后台记录提示外接识别异常，或 `403 block` 排查到 Header 层时，再手动补：
...
这类场景建议主动检查 `User-Agent`，因为很多 HTTP 客户端默认值很短，或者代理会覆盖上游 Header。
...
浏览器通常会自动带自己的 `User-Agent`。前端 JavaScript 一般不能随意修改浏览器的真实 UA；如果你看到客户端里有 Header 设置，也要确认最终请求是不是经过后端代理转发。
...
如果你是浏览器扩展或自建前端，优先看后台请求记录和代理日志，不要只看浏览器控制台里的本地配置。

## 403 时的排查顺序

如果外接请求返回 `403 block`、`403 Forbidden` 或类似拦截提示，按这个顺序查：

| 顺序 | 检查项 | 怎么判断 |
| --- | --- | --- |
| 1 | Key | API Key 是否复制完整、没有过期、来自 UseGoodAI 后台，并且请求使用 `Bearer sk-xxx` |
| 2 | Base URL | 普通 OpenAI-compatible 手动配置时通常是 `https://api.usegoodai.com/v1`；Responses 客户端按当前教程或生成配置填写 |
| 3 | 模型 | 模型名是否和后台当前可用模型逐字一致，Key 所属权限是否支持这个模型 |
| 4 | 接口路径 | Codex / Responses 看实际请求是否到 `/v1/responses`；普通聊天客户端看是否到 `/v1/chat/completions` |
| 5 | `User-Agent` 是否存在 | 后台请求记录或代理日志里能否看到最终发出的 UA |
...
::: tip 先看最终请求
如果你用了代理、网关或自建后端，客户端里填了 `User-Agent` 不代表 UseGoodAI 最终收到了它。排查时优先看 UseGoodAI 后台请求记录，或者看代理发出请求时的最终 Header。
:::

## 给客服的信息

如果排查后仍不能解决，请把这些信息一起发给客服：
...
| 后台请求记录 | 接口路径、状态码、是否有调用记录 |
| 手动设置的 `User-Agent` | 如果没手动设置，就说明“使用客户端默认 UA” |
