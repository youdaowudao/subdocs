...
UseGoodAI 同时支持 **Responses 模式** 和常见 **OpenAI-compatible 兼容模式**。协议会影响接口路径、请求体结构、工具调用和客户端解析方式，选择时先看客户端支持什么协议，不要只看模型名。
...
| 调试时看什么 | 后台日志里看 `/v1/responses`、`output`、`previous_response_id` 等特征 | 后台日志里看 `/v1/chat/completions`、`messages`、`choices` 等特征 |
...
`/v1/responses` 和 `/v1/chat/completions` 是客户端最终请求到 UseGoodAI 时的实际接口路径，由客户端自动拼接。只有客户端明确要求填写完整接口地址时，才按它的说明填写完整路径。
:::
...
| 接入类型 | 使用模式 | 常见填写方式 | 说明 |
| --- | --- | --- | --- |
| Codex | Responses 模式 | 优先复制 UseGoodAI / Codex 教程生成的配置；需要手动时按 Codex 页面配置 `Base URL` 和 Responses 模式 | 后台日志应出现 `/v1/responses` |
| 支持 Responses API 的 Agent / SDK | Responses 模式 | 选择 Responses API 或对应的 Agent 接入方式，`Base URL` 使用 `https://api.usegoodai.com/v1` | 客户端允许选择 API 类型时，不要误选成 Chat Completions |
...
客户端本身只支持 OpenAI-compatible / Chat Completions 时，不要为了“更高级”强行改成 Responses。结果不是能力变强，而是请求体不匹配、工具调用失效或客户端无法解析回复。

使用 Codex 或明确支持 Responses 的 Agent 时，不要退回普通兼容模式。退回后可能还能聊天，但工具调用、状态管理、推理摘要、上下文透传等能力可能不完整。
