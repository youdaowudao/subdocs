...
- 如果客户端支持“压缩上下文”“总结会话”“清理历史消息”，先压缩再发送。
- 如果是代码、日志、文档分析，不要一次性整包提交；按文件、模块或错误片段分批发送。
- 如果确实需要长上下文，换成后台模型列表里明确支持更大上下文的模型，并确认当前 Key 所属分组允许使用该模型。
- 如果换大上下文模型后仍报错，继续减少输入和预留输出长度；文本生成模型限制的是输入和输出合计长度，不只是用户当前打出来的那一段文字。
...
- 后台是否有这次请求记录；如果完全没有记录，优先查 Base URL、代理、客户端配置和本地网络。
- 如果有记录，看请求耗时是在很短时间内取消，还是等待很久后取消。
...
- 如果只在长问题或长输出时出现，减少输入长度，降低输出长度，或关闭客户端里的自动重试/并发请求后再试。
- 如果客户端有超时设置，把请求超时时间调长；代理、网关、反代、Docker、浏览器插件也可能有自己的超时。
- 如果只在流式输出时出现，先关闭 stream 验证非流式请求是否稳定；非流式稳定时，再检查客户端或代理是否会中断 SSE 流。
- 如果后台显示上游长时间无响应，可以临时切换到响应更快的模型，或稍后再试。
- 如果短问题也稳定出现 `499`，把客户端完整报错、后台请求记录、请求时间点、模型名和是否使用代理一起反馈，重点排查客户端到中转站这一段连接是否提前断开。
...
- 通常表示：当前请求的输入、历史消息、工具内容和预期输出合计超过模型上下文上限。不同上游可能返回 `400`、`502 upstream_error` 或其它包装后的状态码，判断时以错误信息和后台上游原文为准。
- 先查什么：后台记录里的模型名、上游原始错误、用户是否长时间未新开会话、是否一次性提交了大文件/长日志/网页全文/大量代码。
- 怎么处理：新开会话；压缩或删除历史上下文；把大文件拆成小段；降低输出长度；需要长上下文时，换后台明确支持更大上下文且当前 Key 分组允许使用的模型。
