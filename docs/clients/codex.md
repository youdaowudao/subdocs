...
Codex 是 OpenAI 的编程助手，可以帮你阅读代码、修改代码、解释报错、生成脚本、整理项目文件，也可以在你的电脑环境里配合 Git、终端和编辑器完成开发任务。简单理解：它不是普通聊天机器人，而是更偏向“帮你处理代码和项目”的 AI 工具。

第一次接触 Codex 的用户，建议优先使用 **Codex App**。App 有图形界面，操作更直观，界面也更完整，适合不熟悉命令行的用户。非程序员一般不需要看 CLI 安装和命令行排障内容，按 [快速开始](/quick-start) 配好 Codex App 即可。
