...
if ($Architecture.ToUpperInvariant() -ne "AMD64") {
    throw ("安装失败：V0.3-r2 仅支持 64 位 x64 Windows，当前架构为 " + $Architecture + "。")
}
...
try {
    ...
    Download-WithPercent -Uri "$ReleaseBase/$Artifact" -Destination $BinaryPath -Activity "正在下载中转站生图工具 V0.3-r2"
