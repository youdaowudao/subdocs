...
if ($Architecture.ToUpperInvariant() -ne "AMD64") {
    throw ("卸载失败：V0.2-r3 仅支持 64 位 x64 Windows，当前架构为 " + $Architecture + "。")
}
...
try {
    ...
    Download-WithPercent -Uri "$ReleaseBase/$Artifact" -Destination $BinaryPath -Activity "正在准备卸载中转站生图工具 V0.2-r3"
