...
if ! command -v curl >/dev/null 2>&1; then
  echo "卸载失败：未找到 curl，无法下载卸载包。" >&2
  exit 1
fi

if ! command -v python3 >/dev/null 2>&1; then
  echo "卸载失败：未找到 Python 3。请先安装 Python 3，再重新运行卸载命令。" >&2
  exit 1
fi
...
if digest.hexdigest() != expected_hash:
    raise SystemExit("卸载失败：卸载包 SHA-256 校验不通过，已停止卸载。")
...
with zipfile.ZipFile(archive_path) as archive:
    for member in archive.infolist():
        ...
        if member_path.is_absolute() or ".." in member_path.parts:
            raise SystemExit("卸载失败：卸载包内包含不安全路径，已停止卸载。")
        if stat.S_ISLNK(mode):
            raise SystemExit("卸载失败：卸载包内包含符号链接，已停止卸载。")
    archive.extractall(unpack_path)
PY

uninstaller_path="$unpack_path/$package_root_name/installer/uninstall.sh"
if [[ ! -f "$uninstaller_path" ]]; then
  echo "卸载失败：卸载包内缺少卸载入口。" >&2
  exit 1
fi
