...
Codex 保留 ChatGPT 登录状态时，可以继续使用 App 内置的 `image_gen` 工具。单独把模型接口改成 OpenAI-compatible，只能让普通对话、代码任务、文件修改走 UseGoodAI，不能自动给当前会话增加内置生图工具。
...
按 [保持 ChatGPT 登录同时连接中转站](/chatgpt-login-usegoodai) 配好后，再回到 Codex 测试内置生图。
