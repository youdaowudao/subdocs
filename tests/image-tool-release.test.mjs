...
const releaseRoot = new URL(
  'install/usegoodai-imagines-tool/releases/v0.7.0/',
  publicRoot,
)
const artifactNames = [
  'usegoodai-imagines-tool-v0.7.0-windows-amd64.exe',
  'usegoodai-imagines-tool-v0.7.0-darwin-arm64',
]
...
test('publishes matching V0.7.0 native artifacts, scripts and checksums', async () => {
  ...
  assert.deepEqual(await readdir(releasesRoot), ['v0.7.0'])
  ...
  for (const script of [shell, powershell]) {
    assert.match(script, /releases\/v0\.7\.0/)
...
test('publishes explicit content types and cache policy', async () => {
  ...
  assert.match(
    headers,
    /\/install\/usegoodai-imagines-tool\/releases\/v0\.7\.0\/usegoodai-imagines-tool-\*\s+Content-Type:\s*application\/octet-stream\s+Cache-Control:\s*public,\s*max-age=31536000,\s*immutable/i,
  )
  assert.match(
    headers,
    /\/install\/usegoodai-imagines-tool\/releases\/v0\.7\.0\/SHA256SUMS\s+Content-Type:\s*text\/plain;\s*charset=utf-8\s+Cache-Control:\s*public,\s*max-age=31536000,\s*immutable/i,
  )
})
