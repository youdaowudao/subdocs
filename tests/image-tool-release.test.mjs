...
const releaseRoot = new URL(
  'install/usegoodai-imagines-tool/releases/v0.8.0/',
  publicRoot,
)
const artifactNames = [
  'usegoodai-imagines-tool-v0.8.0-windows-amd64.exe',
  'usegoodai-imagines-tool-v0.8.0-darwin-arm64',
  'usegoodai-imagines-tool-v0.8.0-darwin-amd64',
  'usegoodai-imagines-tool-v0.8.0-linux-amd64',
]
...
test('publishes matching V0.8.0 native artifacts, scripts and checksums', async () => {
  ...
  assert.deepEqual(
    ...
    ['v0.7.2', 'v0.7.3', 'v0.7.4', 'v0.7.5', 'v0.7.6', 'v0.8.0'],
  )
  ...
  for (const script of [shell, powershell]) {
    assert.match(script, /releases\/v0\.8\.0/)
  ...
  assert.match(combined, /linux-amd64/)
  assert.match(shell, /Darwin\/x86_64/)
  assert.match(shell, /sysctl\.proc_translated/)
  assert.match(shell, /expected_binary_format/)
  assert.doesNotMatch(combined, /windows-arm64|linux-arm64/)
})
...
test('publishes explicit content types and cache policy', async () => {
  ...
  assert.match(
    headers,
    /\/install\/usegoodai-imagines-tool\/releases\/v0\.8\.0\/usegoodai-imagines-tool-\*\s+Content-Type:\s*application\/octet-stream\s+Cache-Control:\s*public,\s*max-age=31536000,\s*immutable/i,
  )
  assert.match(
    headers,
    /\/install\/usegoodai-imagines-tool\/releases\/v0\.8\.0\/SHA256SUMS\s+Content-Type:\s*text\/plain;\s*charset=utf-8\s+Cache-Control:\s*public,\s*max-age=31536000,\s*immutable/i,
  )
})
