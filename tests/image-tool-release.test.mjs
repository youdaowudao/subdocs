...
const releaseRoot = new URL(
  'install/usegoodai-imagines-tool/releases/v0.3-r4/',
  publicRoot,
)
const artifactNames = [
  'usegoodai-imagines-tool-v0.3-r4-windows-amd64.exe',
  'usegoodai-imagines-tool-v0.3-r4-darwin-arm64',
]
...
test('publishes matching V0.3-r4 native artifacts, scripts and checksums', async () => {
  ...
  for (const script of [shell, powershell, shellUninstall, powershellUninstall]) {
    assert.match(script, /releases\/v0\.3-r4/)
...
test('publishes explicit content types and cache policy', async () => {
  ...
  assert.match(
    headers,
    /\/install\/usegoodai-imagines-tool\/releases\/v0\.3-r4\/usegoodai-imagines-tool-\*\s+Content-Type:\s*application\/octet-stream\s+Cache-Control:\s*public,\s*max-age=31536000,\s*immutable/i,
  )
  assert.match(
    headers,
    /\/install\/usegoodai-imagines-tool\/releases\/v0\.3-r4\/SHA256SUMS\s+Content-Type:\s*text\/plain;\s*charset=utf-8\s+Cache-Control:\s*public,\s*max-age=31536000,\s*immutable/i,
  )
})
